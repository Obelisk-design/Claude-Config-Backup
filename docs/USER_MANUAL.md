...
1. 启动应用，点击 "设置" 标签
2. 选择存储类型：
   - **GitHub 存储（推荐）**: 使用 GitHub 私有仓库存储备份
   - **SSH 服务器**: 使用自己的 SSH 服务器存储备份
   - **本地存储**: 仅保存到本地目录
...
如果选择 SSH 服务器存储，需要填写：
   - **服务器地址**: SSH 服务器 IP 或域名
   - **SSH 端口**: 默认 22
   - **用户名**: SSH 登录用户名
   - **密码**: SSH 登录密码（加密存储）

填写完成后，点击 "测试连接" 验证配置是否正确。
...
- ✨ 新增 SSH 服务器存储支持
- 🔐 SSH 密码加密存储，使用 AES-256 加密
- 🔗 SSH 连接测试功能，配置时即可验证
- 🔄 连接重试机制（3 次重试，线性退避）
- 💬 用户友好的错误提示信息
- 🧹 代码优化：提取 SSH 辅助函数，消除重复代码
- ✅ 新增 19 个单元测试，覆盖 SSH 存储所有功能
